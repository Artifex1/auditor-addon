...
const fixture_dir = "tests/typescript/fixtures/";
...
fn runPipeline(allocator: std.mem.Allocator, files: []const []const u8) !*pipeline.Pipeline {
    ...
    pipe.* = try pipeline.Pipeline.init(allocator, cfg.getConfig(.typescript));
    try pipe.run(files, false);
    return pipe;
}

fn parseTypeScript(source: []const u8) !struct { tree: *ts.Tree, parser: *ts.Parser } {
    const parser = ts.Parser.create();
    try parser.setLanguage(cfg.Language.typescript.grammarFn()());
...
test "pipeline: simple_funcs — function nodes and call chain" {
    ...
    const files = [_][]const u8{fixture_dir ++ "simple_funcs.ts"};
...
test "pipeline: simple_class — class container with methods" {
    const allocator = std.testing.allocator;
    const files = [_][]const u8{fixture_dir ++ "simple_class.ts"};
    const pipe = try runPipeline(allocator, &files);
    defer {
        pipe.deinit();
        allocator.destroy(pipe);
...
test "metrics: deep_nesting — cognitive complexity" {
    const source = @embedFile("fixtures/deep_nesting.ts");
    const result = try parseTypeScript(source);
    defer result.tree.destroy();
    defer result.parser.destroy();

    const lang_config = cfg.getConfig(.typescript);
