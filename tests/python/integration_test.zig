...
const fixture_dir = "tests/python/fixtures/";
...
fn runPipeline(allocator: std.mem.Allocator, files: []const []const u8) !*pipeline.Pipeline {
    ...
    pipe.* = try pipeline.Pipeline.init(allocator, cfg.getConfig(.python));
    try pipe.run(files, false);
    return pipe;
}

fn parsePython(source: []const u8) !struct { tree: *ts.Tree, parser: *ts.Parser } {
    const parser = ts.Parser.create();
    try parser.setLanguage(cfg.Language.python.grammarFn()());
...
test "pipeline: simple_funcs — function nodes and call chain" {
    ...
    const files = [_][]const u8{fixture_dir ++ "simple_funcs.py"};
...
test "pipeline: simple_class — class container with methods" {
    const allocator = std.testing.allocator;
    const files = [_][]const u8{fixture_dir ++ "simple_class.py"};
    const pipe = try runPipeline(allocator, &files);
    defer {
        pipe.deinit();
        allocator.destroy(pipe);
    ...
    // Methods
    try std.testing.expect(hasNodeNamed(g, "__init__", .callable));
    try std.testing.expect(hasNodeNamed(g, "increment", .callable));
    try std.testing.expect(hasNodeNamed(g, "get", .callable));
    try std.testing.expect(hasNodeNamed(g, "_reset", .callable));
...
test "metrics: deep_nesting — cognitive complexity" {
    const source = @embedFile("fixtures/deep_nesting.py");
    const result = try parsePython(source);
    defer result.tree.destroy();
    defer result.parser.destroy();

    const lang_config = cfg.getConfig(.python);
