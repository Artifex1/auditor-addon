...
const fixture_dir = "tests/go/fixtures/";
...
fn runPipeline(allocator: std.mem.Allocator, files: []const []const u8) !*pipeline.Pipeline {
    ...
    pipe.* = try pipeline.Pipeline.init(allocator, cfg.getConfig(.go));
    try pipe.run(files, false);
    return pipe;
}

fn parseGo(source: []const u8) !struct { tree: *ts.Tree, parser: *ts.Parser } {
    const parser = ts.Parser.create();
    try parser.setLanguage(cfg.Language.go.grammarFn()());
...
test "pipeline: SimpleFuncs — function nodes and call resolution" {
    const allocator = std.testing.allocator;
    const files = [_][]const u8{fixture_dir ++ "SimpleFuncs.go"};
    ...
    try std.testing.expect(hasNodeNamed(g, "Helper", .callable));
...
test "call_chains: SimpleFuncs — a → b → c chain" {
    const allocator = std.testing.allocator;
    const files = [_][]const u8{fixture_dir ++ "SimpleFuncs.go"};
    const pipe = try runPipeline(allocator, &files);
    defer {
...
test "metrics: DeepNesting — cognitive complexity" {
    const source = @embedFile("fixtures/DeepNesting.go");
    const result = try parseGo(source);
    defer result.tree.destroy();
    defer result.parser.destroy();

    const lang_config = cfg.getConfig(.go);
