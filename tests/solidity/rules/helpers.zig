...
pub fn countRuleHits(
    ...
    // AST bridge uses the regular allocator (manages its own ArrayList)
    var bridge = ast_bridge_mod.AstBridge.init(allocator, &pipe.sources);
    defer bridge.deinit();
    ...
    const lua = try lua_adapter.initLua(aa_alloc, aa_alloc, &pipe.graph, &bridge);
    defer lua.deinit();
    ...
    const hits = if (std.mem.eql(u8, metadata.rule_type, "map"))
        try lua_adapter.executeMapRule(lua, &pipe.graph, &bridge, aa_alloc)
    else
        try lua_adapter.executeVisitorRule(lua, &pipe.graph, metadata, &bridge, pipe.lang_config, aa_alloc);
