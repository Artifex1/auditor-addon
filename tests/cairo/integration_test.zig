...
const fixture_dir = "tests/cairo/fixtures/";
...
fn runPipeline(allocator: std.mem.Allocator, files: []const []const u8) !*pipeline.Pipeline {
    ...
    pipe.* = try pipeline.Pipeline.init(allocator, cfg.getConfig(.cairo));
    try pipe.run(files, false);
    return pipe;
}

fn parseCairo(source: []const u8) !struct { tree: *ts.Tree, parser: *ts.Parser } {
    const parser = ts.Parser.create();
    try parser.setLanguage(cfg.Language.cairo.grammarFn()());
...
test "pipeline: simple_funcs — function nodes detected" {
    ...
    const files = [_][]const u8{fixture_dir ++ "simple_funcs.cairo"};
...
test "pipeline: simple_impl — impl as container" {
    const allocator = std.testing.allocator;
    const files = [_][]const u8{fixture_dir ++ "simple_impl.cairo"};
    const pipe = try runPipeline(allocator, &files);
    defer {
        pipe.deinit();
        allocator.destroy(pipe);
    ...
    try std.testing.expect(hasNodeNamed(g, "CounterImpl", .container));
...
test "metrics: deep_nesting — cognitive complexity" {
    const source = @embedFile("fixtures/deep_nesting.cairo");
    const result = try parseCairo(source);
